...
import { TODOS_UI_URI } from "../types.js";
// =============================================================================
// Helpers
// =============================================================================
/**
 ...
 */
const generateTodoId = () => `todo_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
/**
 * Get all todos from a store, sorted by creation date (newest first).
 */
const getAllTodos = async (store) => {
 ...
 * Add a new todo item.
 */
const handleAdd = async ({ text }, { store }) => {
    if (!text) {
        return {
 ...
 */
const handleToggle = async ({ id }, { store }) => {
    if (!id) {
        return {
 ...
 * Remove a todo item.
 */
const handleRemove = async ({ id }, { store }) => {
    if (!id) {
        return {
            data: { error: "id is required" },
 ...
 */
export const registerTodoTool = (app) => {
    app.tool("todo", {
