...
import { NOTE_UI_URI } from "../types.js";
// =============================================================================
// Helpers
// =============================================================================
const generateNoteId = () => `note_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
/**
 * Get all notes from a store, sorted by most recently updated.
 */
const getAllNotes = async (store) => {
 ...
 */
const extractScope = async (creatureToken) => {
...
const handleOpen = async ({ noteId, title, content }, { setState, store }) => {
    // Open existing note
    if (noteId) {
...
const handleRead = async ({ noteId }, { store }) => {
    if (!noteId) {
...
const handleSave = async ({ noteId, title, content }, { store }) => {
    if (!noteId) {
...
const handleList = async (_input, { store }) => {
...
const handleDelete = async ({ noteId }, { store }) => {
    if (!noteId) {
 ...
 */
export const registerNoteTool = (app) => {
    app.tool("note", {
...
- delete: Remove a note by ID.`,
        ...
        const actionContext = { ...context, store };
